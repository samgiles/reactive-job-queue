 ...
 * @method set
 */
Queue.prototype.set = function(identifier, data, state, cb) {
    ...
    if(state) {
        this._setState(identifier, data.state, data, cb);
    } else {
        this.stateTransitionFunction(identifier, null, function(err, state) {
            this._setState(identifier, state, data, cb);
        }.bind(this));
    }
};

Queue.prototype.setState = function(identifier, newState, cb) {
    this._setState(identifier, newState, null, cb);
};

Queue.prototype._setState = function(identifier, newState, data, cb) {
    data = data || {};

    var firstStateQueue = this.queueName + "-" + newState;
    var queueItemData = { state: newState, data:  JSON.stringify(data)}
    ...
    this.redisClient.hexists([identifier, 'data'], function(err, res) {
        if (res === 0) {
...
Queue.prototype.done = function(identifier, expectedState, callback) {
    ...
    this.redisClient.hexists([identifier, 'data'], function(err, res) {
        if (res === 1) {
            this.redisClient.hmget([identifier, 'state', 'data'], function(err, data) {
...
Queue.prototype.has = function(identifier, callback) {
    this.redisClient.hexists([identifier, 'data'], function(err, result) {
        if (err) {
            callback(err);
            return;
        }
...
Queue.prototype._stateTransition = function(identifier, transition, callback) {
    ...
    var eventObject = { id: identifier, from: transition.from };
    ...
    this.redisClient.multi(redisMultiCommands).exec(function(err, result) {
        if (err) {
            ...
            this.emit('transition', eventObject);
