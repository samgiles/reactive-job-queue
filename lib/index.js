...
function ReactiveJobQueue(options) {
    ...
    this.queueName = "__rjq-" + options.queuename;
...
ReactiveJobQueue.prototype.send = function(job, callback) {
	if (job === null || typeof job !== 'object') {
		process.nextTick(function() { callback("Job must be an object"); });
		return;
	}
...
ReactiveJobQueue.prototype.waitQueueLength = function(callback) {
...
ReactiveJobQueue.prototype._stateTransition = function(job, transition, callback) {
    var transitionFromQueue = this.queueName + "-" + transition.from;
    var transitionToQueue = this.queueName + "-" + transition.to;
    var jobString = JSON.stringify(job);

    this.redisClient.multi([
        ['lrem', transitionFromQueue, jobString],
