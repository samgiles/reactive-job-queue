...
Queue.prototype.send = function(identifier, data, cb) {
    ...
    this.stateTransitionFunction(identifier, null, function(err, newState) {
        ...
        var queueItemData = { id: identifier, state: newState, data: data };
...
Queue.prototype.done = function(identifier, expectedState, callback) {
    ...
    this.redisClient.hexists([this.hashSetName, identifier], function(err, res) {
        if (res === 1) {
            this.redisClient.hget([this.hashSetName, identifier], function(err, data) {
                var object = JSON.parse(data);

                if (object.state !== expectedState) {
                    callback(new Error("Current state is not the expected state. identifier=" + identifier + " actualstate=" + object.state + " expectedstate=" + expectedState));
                    return;
                }

                this.stateTransitionFunction(identifier, object.state, function(err, newState) {
                    this._stateTransition(identifier, { from: object.state, to: newState }, callback);
                }.bind(this));
            }.bind(this));
        } else {
...
Queue.prototype.has = function(identifier, callback) {
    this.redisClient.hexists([this.hashSetName, identifier], function(err, result) {
        if (err) {
            callback(err);
            return;
        }
